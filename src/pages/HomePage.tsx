...
export function HomePage() {
  ...
  // Simulation: Checking if there is a live session using current time logic
  ...
  const mappedServices = [
    { title: 'Weekly Services', desc: 'Join us every Sunday for powerful worship.' },
  ...
  return (
    <div className="flex flex-col bg-white overflow-hidden pb-12">
      ...
      <section className="relative w-full pt-48 pb-64 flex flex-col justify-center min-h-[60vh] md:min-h-[80vh] bg-[#0b162c] text-white isolate">
        ...
        <div className="relative z-10 w-full max-w-[1400px] mx-auto px-6 md:px-16">
          ...
          >
            ...
            <motion.h1 variants={fadeInUp} className="font-display font-bold text-5xl md:text-7xl leading-[1.05] tracking-tight mb-4">
              {siteContent.site.heroTitle || 'Welcome To Our Church'}
            </motion.h1>
            <motion.p variants={fadeInUp} className="text-white/70 text-base md:text-lg leading-relaxed max-w-xl mb-8">
              {siteContent.site.welcomeMessage || 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Urna condimentum mattis pellentesque id nibh tortor.'}
            </motion.p>
            
            <motion.div variants={fadeInUp}>
              <Link className="inline-block px-8 py-3.5 bg-[#f97316] text-white font-medium text-sm hover:bg-[#ea580c] transition-colors" to="/plan-your-visit">
                I'm New Here
              </Link>
            </motion.div>
          </motion.div>
      ...
      {/* 2. Overlapping Live Stream Box */}
      {isLive && (
        <section className="relative z-20 max-w-[1000px] mx-auto w-full px-6 -mt-[4rem] md:-mt-[5rem] mb-24 hidden md:block">
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.4, duration: 0.8 }}
            className="bg-white shadow-[0_20px_60px_rgba(0,0,0,0.08)] flex flex-col md:flex-row items-center rounded-sm overflow-hidden"
          >
             <div className="w-full md:w-1/3 h-48 md:h-auto">
               <img src="https://images.unsplash.com/photo-1543807535-eceef0bc6599?q=80&w=600&auto=format&fit=crop" className="w-full h-full object-cover" alt="Live stream thumbnail" />
             </div>
             <div className="w-full md:w-2/3 p-8 flex flex-col md:flex-row items-center justify-between gap-6 bg-white">
                <div>
                   <h3 className="font-display text-2xl font-bold text-stone-900 mb-1">Join the Sunday Live Stream</h3>
                   <p className="text-stone-500 text-sm">Every Sunday at 9:00 am & 11:00 am</p>
                </div>
                <div className="flex items-center gap-6 shrink-0">
                  <a href={siteContent.site.watchOnlineUrl} target="_blank" rel="noreferrer" className="inline-block px-6 py-2.5 bg-[#f97316] text-white text-sm font-medium hover:bg-[#ea580c] transition-colors">
                    Join Now →
                  </a>
                  <Link to="/sermons" className="text-stone-500 text-sm font-medium hover:text-stone-900 transition-colors">
                    Past Messages
                  </Link>
                </div>
             </div>
